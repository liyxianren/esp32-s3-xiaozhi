...
// 导出固件版本供串口命令使用
const char* FIRMWARE_VERSION = "v2.0.3-reminder-20251101-v3";
...
Application::Application() {
    // 打印固件版本信息
    ESP_LOGI(TAG, "========================================");
    ESP_LOGI(TAG, "🚀 XIAOZHI FIRMWARE VERSION: %s", FIRMWARE_VERSION);
    ESP_LOGI(TAG, "📅 BUILD DATE: %s %s", __DATE__, __TIME__);
    ESP_LOGI(TAG, "✨ FEATURES: ReminderManager enabled");
    ESP_LOGI(TAG, "========================================");
...
#error "CONFIG_USE_DEVICE_AEC and CONFIG_USE_SERVER_AEC cannot be enabled at the same time"
    ...
    reminder_manager_.SetCallback([this](uint32_t id, const std::string& message) {
        ESP_LOGI(TAG, "🔔 Reminder callback invoked: id=%u, message='%s'", id, message.c_str());
        this->OnReminderTriggered(id, message);
    });
    ESP_LOGI(TAG, "✅ ReminderManager callback registered");

    // 创建10秒后打印版本号的定时器
    esp_timer_handle_t version_timer;
    esp_timer_create_args_t version_timer_args = {
        .callback = [](void* arg) {
            ESP_LOGI(TAG, "");
            ESP_LOGI(TAG, "╔════════════════════════════════════════╗");
            ESP_LOGI(TAG, "║  📌 FIRMWARE VERSION CHECK            ║");
            ESP_LOGI(TAG, "╠════════════════════════════════════════╣");
            ESP_LOGI(TAG, "║  Version: %-28s ║", FIRMWARE_VERSION);
            ESP_LOGI(TAG, "║  Build:   %s %s      ║", __DATE__, __TIME__);
            ESP_LOGI(TAG, "║  Features: ReminderManager + Custom   ║");
            ESP_LOGI(TAG, "╚════════════════════════════════════════╝");
            ESP_LOGI(TAG, "");
            // 删除定时器，只触发一次
    ...
    esp_timer_create(&version_timer_args, &version_timer);
    // 传递定时器句柄给回调，以便自删除
    *((esp_timer_handle_t*)&version_timer_args.arg) = version_timer;
    // 10秒后触发
    esp_timer_start_once(version_timer, 10 * 1000000);  // 10秒 = 10,000,000 微秒
    ESP_LOGI(TAG, "⏰ Version print timer set for 10 seconds");
}
...
void Application::OnReminderTriggered(uint32_t id, const std::string& message) {
    ESP_LOGI(TAG, "⏰⏰⏰ OnReminderTriggered: id=%u, message='%s'", id, message.c_str());
    ESP_LOGI(TAG, "Current device state: %d", device_state_);
    ...
    ESP_LOGI(TAG, "✅ Reminder event bits set, waiting for main loop to process");
}